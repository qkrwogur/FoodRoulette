...
public class ladder extends AppCompatActivity {
    TextView mTextMax = null;//textView 선언 하고 초기화
    //이 textView는 인원 수를 표시하는 textView다.
    int mPeopleMax = 2;//초기 인원수는 2명이다.
    // ListView 에 표시할 데이터를 저장하는 ArayList 배열
    ArrayList<MyItem> mArMember; //구조체이다
    ListView mListMember;
    MyListAdapter MyAdapter = null;//클래스이다


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.ladder_main);
    ...
    public void startLadderGame() {
        Intent intent = new Intent(getApplicationContext(), ladderGame.class);
